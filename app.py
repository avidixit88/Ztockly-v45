...
with st.expander("Replay mode (historical bar-by-bar)", expanded=False):
    ...
    rep_symbol = st.text_input("Replay ticker", value=(watchlist[0] if watchlist else "AAPL"))
    ...
    if run_replay_btn:
        bundle = fetch_bundle(rep_symbol, interval=interval)
